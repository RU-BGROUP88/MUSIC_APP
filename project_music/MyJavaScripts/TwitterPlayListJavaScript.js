$(document).ready(function () {
    ...
    GetTopAlbums();
    GetTweetsfromDB();
    $("#TwitterForm").submit(f1);

    //$('#Savebutton').on('click', AddTweetToDB);
...
function errorGetTweets(err) {
    swal("GetTweets got wrong!" + err, "", "error");
}
...
function errorGetTopArtists(data) {
    $("#Demo1").empty();
...
function errorGetTopTracks(data) {
    $("#Demo2").empty();
...
function GetTweetsfromDB() {
    ajaxCall("GET", "../api/Twitter/?UserId=" + UserId, "", successGetTweets, errorGetTweets);
...
function successGetTweets(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        str += "<p>" + data[i].Date + "</p>";
        str += "<button type=submit onclick='RemoveTweetFromDB(" + i + ")'  value= " + i + ">Remove</button>"
...
function RemoveTweetFromDB(ObjectNumber) {
    TweetID = tweets[ObjectNumber].TweetID;
    ajaxCall("Put", "../api/Twitter/?TweetID=" + TweetID, "", successRemoveTweet, errorRemoveTweet);


}
function successRemoveTweet() {
...
function errorRemoveTweet() {
    swal("Remove from playlist going wrong", "App Music Thank you for registering ", "error");
